...
class ModelExtensionModuleRedirect extends Model {
	...
	{
		$this->db->query("INSERT INTO " . DB_PREFIX . "redirect SET url_from = '" . $this->db->escape($data['url_from']) . "', url_to = '" . $this->db->escape($data['url_to']) . "'");
	...
	{
		$this->db->query("UPDATE " . DB_PREFIX . "redirect SET url_from = '" . $this->db->escape($data['url_from']) . "', url_to = '" . $this->db->escape($data['url_to']) . "' WHERE redirect_id = '" . (int)$redirect_id . "'");
	}
	...
	{
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "redirect WHERE redirect_id = '" . (int)$redirect_id . "'");
		return $query->row;
	}

	public function existsUrlFrom($url_from) 
	{
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "redirect WHERE url_from = '" . $this->db->escape($url_from) . "'");
	...
	public function install() {
		...
		"redirect` ( `redirect_id` int(11) NOT NULL AUTO_INCREMENT, `url_from` varchar(150) NOT NULL, `url_to` varchar(150) NOT NULL, PRIMARY KEY (`redirect_id`) )
		ENGINE=MyISAM  DEFAULT CHARSET=utf8;";
		$this->db->query($create_table_sql);
	}

	public function uninstall() {
		$delete_table_sql = "DROP TABLE IF EXISTS `" . DB_PREFIX . "redirect`";
